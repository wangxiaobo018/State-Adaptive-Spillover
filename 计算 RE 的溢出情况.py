...
##################### all_RM 选择 3，all_RD 选择 2 all_RP 选择2 ################3
...
def main():
    """
    主执行函数,聚焦于分析和总结对BTC的溢出贡献。
    """
    TARGET_VAR = 'BTC'
    model = TVP_QVAR_DY(data_df)

    quantiles_to_run = [0.01, 0.05, 0.10, 0.25, 0.50, 0.75, 0.90, 0.95, 0.99]
    key_quantiles_for_summary = [0.05, 0.10, 0.50, 0.90, 0.95]

    print("=" * 60)
    print(f"      核心分析: 其他加密货币对{TARGET_VAR}的溢出贡献")
    print("=" * 60)

    # 运行核心分析
    spillover_to_btc, raw_summary_df = analyze_spillover_to_btc(
        model,
        quantiles=quantiles_to_run,
        nlag=3,
        nfore=10,
        save_results=True
    )
    ...
    # 生成并打印格式化的最终总结表格
    print("\n" + "=" * 70)
    print(f"         各加密货币在关键分位数下对{TARGET_VAR}的溢出贡献率总结")
    print("=" * 70)

    summary_data_list = []
    # 【修正点4】确保这里的过滤逻辑正确
    source_vars = [name for name in data_df.columns if name != TARGET_VAR]

    for var in source_vars:
        row = {'加密货币': var}
        for q in key_quantiles_for_summary:
            if q in raw_summary_df.index:
                spillover_value = raw_summary_df.loc[q, var]
                row[f'τ={q:.2f} (%)'] = f"{spillover_value:.2f}"
            else:
                row[f'τ={q:.2f} (%)'] = 'N/A'
        try:
            low_val = raw_summary_df.loc[0.05, var]
            high_val = raw_summary_df.loc[0.95, var]
            diff = high_val - low_val
            row['周期属性 (0.95-0.05)'] = f"{diff:+.2f}"
        except KeyError:
            row['周期属性 (0.95-0.05)'] = 'N/A'
        summary_data_list.append(row)

    final_summary_df = pd.DataFrame(summary_data_list)
