...
# Check for infinite or NA values in all_RD
RD_inf = np.isinf(all_RD)
RD_nan = np.isnan(all_RD)
all_RD = np.where(RD_inf | RD_nan, 0, all_RD)

# Check for infinite or NA values in all_RP
RP_inf = np.isinf(all_RP)
RP_nan = np.isnan(all_RP)
all_RP = np.where(RP_inf | RP_nan, 0, all_RP)
